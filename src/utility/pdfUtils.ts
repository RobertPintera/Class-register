...
async function loadSvgToCanvas(src: string): Promise<HTMLCanvasElement> {
  const img = new Image();
  img.src = src;
...
export async function generateStudentReportPDF (report: StudentReport) {
  const doc = new jsPDF();
  ...
  // Logo
  const canvas = await loadSvgToCanvas("/logo_class_register.svg");
